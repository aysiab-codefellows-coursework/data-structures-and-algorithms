...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach((value) => {
    ...
    let addTemplate = $(`<section>${template}<section>`);
    addTemplate.find('h2').text(value.name);
    addTemplate.find('h3').text(value.height);
    addTemplate.find('p').text(value.eye_color);
    $('main').append(addTemplate);
  });
