...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let ret = arr.reduce((accumulator) => {
    accumulator += 1;
    return accumulator;
  });
  return ret;
...
const returnNames = (arr) => {
  // Solution code here...
  let ret = arr.reduce((accumulator, person) => {
...
const reversedString = (str) => {
  // Solution code here...
  let arr = str.split('');
  let ret = arr.reduce((accumlator, value, index, src) => {
    accumlator = `${accumlator}${src[src.length-(index+1)]}`;
    return accumlator;
  }, "");
  return ret;
};
