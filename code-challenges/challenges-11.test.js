...
const toLastNames = people => {
  // Solution code here...
  let ret = people.map(person => `${person.firstName} ${person.lastName}`);
  return ret;
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let validPhoneNum = /^\(?(\d{3})\)?[- ]?(\d{3})[- ]?(\d{4})$/;
  return validPhoneNum.test(phoneNumber);
};
