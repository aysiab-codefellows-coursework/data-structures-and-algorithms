...
const createServer = () => {
  ...
  app.get('/hello',(request, response) => response.send('Hello from the otherside!'));
  app.get('/aboutme', (request, response) => response.send('I am a cat mom to two cats, Midna & Kiara.'));
  app.get('/favoritefoods', (request, response) => response.json({'fave foods': ['Spaghetti','Fried Rice','Ice Cream']}));
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let ret = input.map(arrays => {
    let arrs = arrays.filter(nums => {
      return Number.isInteger(nums) && nums % 5 === 0;
    }).map(nums => Math.pow(2, nums));
    return arrs;
  });
  console.log('ret', ret);
  return ret;
