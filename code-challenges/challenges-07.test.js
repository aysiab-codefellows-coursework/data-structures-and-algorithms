...
const mapTwoToThe = (arr) => {
  // Solution code here...
  let ret = arr.map(num => Math.pow(2,num));
  return ret;
};
...
const charCode = (arr) => {
  // Solution code here...
  let ret = arr.map(char => char.charCodeAt(0));
  return ret;
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  let ret = arr.map(move => move.ability.name);
  return ret;
};
